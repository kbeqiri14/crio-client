    ...
    () =>
      data?.getProduct
        ? {
            ...
            isProduct: true,
            thumbnails:
              data.getProduct.thumbnails?.length > 1
                ? data.getProduct.thumbnails?.map((item) =>
                    getThumbnail(PRODUCTS, data.getProduct.userId, `thumbnail-${item}`),
                  )
                : data.getProduct.thumbnails?.[0]
                ? getThumbnail(
                    PRODUCTS,
